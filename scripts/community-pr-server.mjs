...
async function github(path, options = {}) {
  if (!TOKEN) {
    throw new Error('GITHUB_TOKEN ist nicht gesetzt. Bitte einen neuen Token als Umgebungsvariable setzen.');
  }
...
async function createPullRequest({ scenario, suggestedPath, prTitle, prBody }) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method !== 'POST' || req.url !== '/api/community-pr') {
